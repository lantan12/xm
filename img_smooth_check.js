...
WIDGETS.imgSmoothCheck = function (options) {
    ...
    var blnIsMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|opera mini|opera mobile|appleWebkit.*mobile|mobile/i.test(navigator.userAgent);
    ...
    $smoothCnt.on(sSmoothMove, function (evt) {
        ...
        if (flag === true) {
            ...
            if(evt.targetTouches){
                var new_pageX=evt.targetTouches[0].pageX;
            }else{
                var new_pageX=evt.pageX;
            }
            var numPageX = blnIsMobile ? new_pageX : evt.pageX;
            var numMoveLength = 0;
